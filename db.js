...
async function smartFetch(url, options = {}) {
  ...
  try {
    ...
    if (method === 'GET' && response.ok) {
      response.clone().json().then((data) => saveToCache(url, data)).catch(() => {});
    }
  ...
  for (const req of requests) {
    try {
      ...
      if (response.ok) {
        ...
        if (typeof window !== 'undefined' && typeof window.showNetworkToast === 'function') {
        ...
        console.warn(`[SyncEngine] Server rejected sync data: ${req.url} (Status: ${response.status})`);
      }
    } catch (err) {
      break;
    }
